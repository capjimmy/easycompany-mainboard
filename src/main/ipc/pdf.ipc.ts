...
export function registerPdfHandlers(): void {
  ...
  ipcMain.handle('pdf:generateQuote', async (_event, requesterId: string, quoteId: string) => {
    ...
    try {
      ...
      if (requester.role !== 'super_admin' && quote.company_id !== requester.company_id) {
  ...
  // PDF 미리보기 HTML 반환 (견적서)
  ipcMain.handle('pdf:previewQuote', async (_event, requesterId: string, quoteId: string) => {
    const requester = await db.getUserById(requesterId);
    if (!requester) return { success: false, error: '권한이 없습니다.' };
    ...
    try {
      ...
      const html = generateHtml('quote', pdfData);
  ...
  // PDF 미리보기 HTML 반환 (계약서)
  ipcMain.handle('pdf:previewContract', async (_event, requesterId: string, contractId: string) => {
    const requester = await db.getUserById(requesterId);
    if (!requester) return { success: false, error: '권한이 없습니다.' };
    ...
    try {
      ...
      const html = generateHtml('contract', pdfData);
      return { success: true, html };
    } catch (err: any) {
      return { success: false, error: err.message };
    }
  ...
  ipcMain.handle('pdf:searchContacts', async (_event, requesterId: string, clientName: string) => {
    ...
    try {
      ...
      const allClients = requester.role === 'super_admin'
        ? await db.getClientCompanies()
        : await db.getClientCompaniesByCompanyId(requester.company_id);
