...
export function registerOutsourcingHandlers(): void {
  ...
  ipcMain.handle('outsourcings:getAll', async (_event, requesterId: string, filters?: any) => {
    ...
    // super_admin company switch filter
    if (requester.role === 'super_admin' && filters?.company_id) {
      outsourcings = outsourcings.filter((o: any) => o.company_id === filters.company_id);
    }
