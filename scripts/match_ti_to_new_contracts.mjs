// Task 1: 세금계산서 → 계약 재매칭
import { createClient } from '@supabase/supabase-js';
import OpenAI from 'openai';

const sb = createClient('https://silvsqcwearelrumtqqm.supabase.co', 'sb_publishable_pAS3A3nCHvsuS0ew46MD5A_RpnMdt4J');
const { data: keyData } = await sb.from('settings').select('value').eq('key', 'openai_api_key').single();
const openai = keyData?.value ? new OpenAI({ apiKey: keyData.value }) : null;
console.log('OpenAI key loaded:', !!openai);

async function fetchAll(t, c) {
  const all=[]; let f=0;
  while(true){const{data}=await sb.from(t).select(c).order('id').range(f,f+999);if(!data||!data.length)break;all.push(...data);if(data.length<1000)break;f+=1000;}
  return all;
}

function norm(s) {
  if (!s) return '';
  return String(s).toLowerCase()
    .replace(/㈜/g, '')
    .replace(/㈔/g, '')
    .replace(/\(주\)/g, '')
    .replace(/\(유\)/g, '')
    .replace(/\(사\)/g, '')
    .replace(/주식회사/g, '')
    .replace(/유한회사/g, '')
    .replace(/사단법인/g, '')
    .replace(/재단법인/g, '')
    .replace(/[\s\(\)\[\]\{\}（）「」『』""''.,\-_/\\:;]/g, '');
}

const tis = await fetchAll('tax_invoices', 'id, company_id, contract_id, buyer_name, item_description, total_amount, supply_amount, status, issue_date');
const contracts = await fetchAll('contracts', 'id, company_id, contract_number, client_company, service_name, total_amount, contract_amount, contract_date');
console.log(`tax_invoices ${tis.length}, contracts ${contracts.length}`);

const unlinked = tis.filter(t => !t.contract_id);
console.log(`미연결 세금계산서: ${unlinked.length}`);

// 회사별 계약 인덱스
const contractsByCompany = new Map();
for (const c of contracts) {
  const k = c.company_id;
  if (!contractsByCompany.has(k)) contractsByCompany.set(k, []);
  contractsByCompany.get(k).push(c);
}

async function llmMatch(invoice, candidates) {
  if (!openai) return null;
  const list = candidates.slice(0, 15).map((c, i) =>
    `${i+1}. ${c.client_company || '?'} | ${c.service_name || '?'} | ${(c.total_amount||c.contract_amount||0).toLocaleString()}원 | ${c.contract_date || '?'}`
  ).join('\n');
  const prompt = `세금계산서가 어떤 계약에 해당하는지 정확히 매칭해야 합니다.

[세금계산서]
거래처: ${invoice.buyer_name}
적요: ${invoice.item_description || '?'}
금액: ${invoice.total_amount?.toLocaleString()}원
발행일: ${invoice.issue_date}

[계약 후보]
${list}

가장 정확하게 일치하는 계약 번호 1개만 답하세요. 거래처명과 적요/서비스명이 일치할 때 매칭하세요. 불확실하면 0을 답하세요.
답변 형식: 숫자만 (예: 3 또는 0)`;

  try {
    const res = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        { role: 'system', content: '데이터 매칭 전문가. 정확도가 가장 중요. 숫자만 답하세요.' },
        { role: 'user', content: prompt },
      ],
      max_tokens: 5,
      temperature: 0,
    });
    const txt = res.choices[0]?.message?.content?.trim() || '0';
    const num = parseInt(txt.match(/\d+/)?.[0] || '0');
    if (num > 0 && num <= candidates.length) return candidates[num - 1];
    return null;
  } catch (e) { console.log('LLM err:', e.message); return null; }
}

let matched = 0;
let skipped = 0;
let llmCalls = 0;
const matches = [];

for (let i = 0; i < unlinked.length; i++) {
  const inv = unlinked[i];
  const candidates = contractsByCompany.get(inv.company_id) || [];
  const buyerN = norm(inv.buyer_name);
  const descN = norm(inv.item_description);

  if (!buyerN) { skipped++; continue; }

  // 1차: 정확한 거래처명 매칭
  const exact = candidates.filter(c => {
    const cClient = norm(c.client_company);
    return cClient && cClient === buyerN;
  });

  let pool = exact;
  // 2차: 부분 매칭 (4글자 prefix)
  if (pool.length === 0) {
    pool = candidates.filter(c => {
      const cClient = norm(c.client_company);
      if (!cClient) return false;
      const minLen = Math.min(cClient.length, buyerN.length);
      if (minLen < 4) return cClient === buyerN;
      const p1 = buyerN.substring(0, 4);
      const p2 = cClient.substring(0, 4);
      return p1 === p2 || buyerN.includes(cClient.slice(0,4)) || cClient.includes(buyerN.slice(0,4));
    });
  }

  if (pool.length === 0) { skipped++; continue; }

  // 1개 후보 → 바로 매칭
  if (pool.length === 1) {
    matches.push({ id: inv.id, contract_id: pool[0].id });
    matched++;
    if ((matched+skipped) % 50 === 0) process.stdout.write(`${matched+skipped}/${unlinked.length}(M${matched}) `);
    continue;
  }

  // 2+ 후보 → 적요로 좁히기
  if (descN && descN.length >= 4) {
    const descFiltered = pool.filter(c => {
      const cService = norm(c.service_name);
      if (!cService || cService.length < 4) return false;
      return descN.includes(cService.slice(0, 6)) || cService.includes(descN.slice(0, 6));
    });
    if (descFiltered.length === 1) {
      matches.push({ id: inv.id, contract_id: descFiltered[0].id });
      matched++;
      continue;
    }
    if (descFiltered.length >= 2) pool = descFiltered;
  }

  if (pool.length === 1) {
    matches.push({ id: inv.id, contract_id: pool[0].id });
    matched++;
    continue;
  }

  // 2개 이상 → AI 사용
  if (pool.length >= 2) {
    llmCalls++;
    const best = await llmMatch(inv, pool);
    if (best) {
      matches.push({ id: inv.id, contract_id: best.id });
      matched++;
    } else skipped++;
  } else skipped++;

  if ((matched+skipped) % 25 === 0) process.stdout.write(`${matched+skipped}/${unlinked.length}(M${matched},LLM${llmCalls}) `);
}
console.log(`\n매칭 ${matched}, 스킵 ${skipped}, LLM 호출 ${llmCalls}`);

// 적용
let applied = 0;
for (const m of matches) {
  const { error } = await sb.from('tax_invoices').update({ contract_id: m.contract_id }).eq('id', m.id);
  if (!error) applied++;
}
console.log(`✅ ${applied}건 contract_id 업데이트`);

// 검증
const ti2 = await fetchAll('tax_invoices', 'id, contract_id');
const linked = ti2.filter(t => t.contract_id).length;
console.log(`최종: ${linked}/${ti2.length} 매칭 (${Math.round(linked/ti2.length*100)}%)`);
