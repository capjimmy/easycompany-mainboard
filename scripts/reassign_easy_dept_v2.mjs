// Task 2: 이지컨설턴트 부서 자동 배정 (미배정 → 친환경인증 default)
...
if (!greenCert) {
  console.log('친환경인증본부 부서 없음 - departments 목록:', deps.map(d => d.name));
  process.exit(1);
}

const contracts = await fetchAll('contracts', 'id, company_id, department_id, service_name, notes, source_file_path, client_company');
const target = contracts.filter(c => c.company_id === easy.id);
console.log(`이지컨설턴트 계약: ${target.length}건`);

let counts = { greenCert: 0, edu: 0, arch: 0, defaultGreen: 0, alreadyAssigned: 0 };
let updated = 0;

for (const c of target) {
  if (c.department_id) {
    counts.alreadyAssigned++;
    continue;
  }
  const text = `${c.service_name || ''} ${c.notes || ''} ${c.source_file_path || ''} ${c.client_company || ''}`.toLowerCase();
  let deptId = null;
  let bucket = 'defaultGreen';

  if (/친환경|녹색|에너지효율|zeb|본인증|장수명|결로|에너지|esg|친환경인증/i.test(text)) {
    deptId = greenCert; bucket = 'greenCert';
  } else if (/제네시스|교육환경|학교|유치원|초등|중학|고등|교육|에듀|edu/i.test(text)) {
    deptId = edu; bucket = 'edu';
  } else if (/공동주택|아파트|설계|공모|건축사사무소|민간참여|발주처|타워|주거|판상형/i.test(text)) {
    deptId = arch; bucket = 'arch';
  } else {
    // 매칭 안되면 친환경인증본부 default
    deptId = greenCert; bucket = 'defaultGreen';
  }
  counts[bucket]++;

  if (deptId && deptId !== c.department_id) {
    const { error } = await sb.from('contracts').update({ department_id: deptId }).eq('id', c.id);
    if (!error) updated++;
  }
}
